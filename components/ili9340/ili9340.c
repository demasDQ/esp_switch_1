...
void lcdInit(TFT_t * dev, uint16_t model, int width, int height, int offsetx, int offsety)
{
	...
	if (dev->_model == 0x9340 || dev->_model == 0x9341 || dev->_model == 0x7735) {
		...
		spi_master_write_comm_byte(dev, 0x36); //Memory Access Control
		#ifdef CONFIG_RGB_COLOR
		spi_master_write_data_byte(dev, 0xC0); // Right top start, RGB color filter panel, horizontal mirror
		#else
		// ST7735 显示方向设置
		// bit7=MY: 上下反转, bit6=MX: 左右反转, bit5=MV: XY交换
		// 0x08 = 00001000B - 默认设置
		// 0xC0 = 11000000B - 上下颠倒
		// 0xA0 = 10100000B - 左右镜像  
		// 0x60 = 01100000B - 上下颠倒 + 左右镜像
		//0x40;
		spi_master_write_data_byte(dev, 0x08); //根据实际显示效果调整此值
		#endif
